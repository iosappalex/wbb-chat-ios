...
+(void) setTextMessageHeights: (SurespotMessage *)  message size: (CGSize) size {
    ...
    if (plaintext){
        ...
        DDLogVerbose(@"computed row height portrait %ld landscape %ld", message.rowPortraitHeight, (long)message.rowLandscapeHeight);
    }
}
...
+(REMenu *) createMenu: (NSArray *) menuItems closeCompletionHandler: (void (^)(void))completionHandler {
    ...
    menu.itemHeight = 40;
    menu.backgroundColor = [UIColor colorWithString:NAVBAR_COLOR];
    ...
    menu.separatorColor = [UIColor colorWithString:@"#243545"];
    menu.highlightedSeparatorColor = [[UIUtils surespotTransparentBlue] colorWithAlphaComponent:0.5f];
    menu.bounce = NO;
    menu.borderWidth = 0;
