...
+ (NSData *) signData1: (NSData *) data1 data2: (NSData *) data2 withPrivateKey: (ECDSAPrivateKey *) privateKey {
    ...
    unsigned long sigLength = signer.MaxSignatureLength();
    
    byte * signature = new byte[sigLength];
    unsigned long sigLen = signer.SignMessage(randomRng, (byte *)[concatData bytes], concatData.length, signature);
    
    byte * buffer = new Byte[1000];
    unsigned long put = CryptoPP::DSAConvertSignatureFormat(buffer, 1000, CryptoPP::DSASignatureFormat::DSA_DER, signature, sigLen, CryptoPP::DSASignatureFormat::DSA_P1363);
...
+(BOOL) verifyPublicKeySignature: (NSData *) signature data: (NSString *) data {
    ...
    unsigned long put = CryptoPP::DSAConvertSignatureFormat(buffer,verifier.SignatureLength(), CryptoPP::DSASignatureFormat::DSA_P1363, reinterpret_cast<unsigned char const* >([signature bytes]), [signature length], CryptoPP::DSASignatureFormat::DSA_DER);
