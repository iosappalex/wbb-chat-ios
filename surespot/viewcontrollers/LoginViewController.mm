...
{
    [super viewDidLoad];
    [_textPassword setPlaceholder:[NSLocalizedString(@"password", nil) stringWithSentenceCapitalization]];
    [self.navigationItem setTitle:[NSLocalizedString(@"login", nil) stringWithSentenceCapitalization]];
    ...
    _storeKeychainLabel.text = [NSLocalizedString(@"store_password_in_keychain", nil) stringWithSentenceCapitalization];
    ...
    dispatch_async(q, ^{
        ...
        [[NetworkController sharedInstance]
         ...
         successBlock:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON, NSHTTPCookie * cookie) {
             DDLogVerbose(@"login response: %ld",  [response statusCode]);
...
-(REMenu *) createMenu {
    ...
    REMenuItem * createItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"create_identity_label", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_add"] highlightedImage:nil action:^(REMenuItem * item){
        if ([[IdentityController sharedInstance] getIdentityCount] < MAX_IDENTITIES) {
            [self performSegueWithIdentifier: @"createSegue" sender: self];
        }
        else {
            [UIUtils showToastMessage:[NSString stringWithFormat: [NSLocalizedString(@"login_max_identities_reached",nil) stringWithSentenceCapitalization], MAX_IDENTITIES] duration:2];
        }
    }];
    ...
    REMenuItem * clearCacheItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"clear_local_cache", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_delete"] highlightedImage:nil action:^(REMenuItem * item){
