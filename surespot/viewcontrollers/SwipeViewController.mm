...
- (void)viewDidLoad
{
    ...
    UIImage * backImage = [UIImage imageNamed:@"app_icon"];
    ...
    UIBarButtonItem *anotherButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"more_icon"] style:UIBarButtonItemStyleBordered target:self action:@selector(showMenuMenu)];
...
-(NSString * ) titleForLabelForPage:(NSInteger)page {
    DDLogVerbose(@"titleForLabelForPage %ld", page);
    if (page == 0) {
        return @"home";
    }
    else {
...
{
    DDLogVerbose(@"view for item at index %ld", index);
    if (index == 0) {
        if (!_friendView) {
            ...
            _friendView.tableFooterView = [UIView new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (index == 0) {
        ...
        if (count == 0) {
            static NSString *CellIdentifier = @"Cell";
            UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
            cell.textLabel.text = [NSLocalizedString(@"no_friends", nil) stringWithSentenceCapitalization];
        ...
        if (afriend.isInvited || afriend.isInviter || afriend.isDeleted) {
            ...
            if (afriend.isDeleted) {
                cell.friendStatus.text = [NSLocalizedString(@"friend_status_is_deleted", nil) stringWithSentenceCapitalization];
            }
            
            if (afriend.isInvited) {
                cell.friendStatus.text = [NSLocalizedString(@"friend_status_is_invited", nil) stringWithSentenceCapitalization];
            }
            
            if (afriend.isInviter) {
                cell.friendStatus.text = [NSLocalizedString(@"friend_status_is_inviting", nil) stringWithSentenceCapitalization];
                [cell.blockButton setTitle:[NSLocalizedString(@"block_underline", nil) stringWithSentenceCapitalization] forState:UIControlStateNormal];
                [cell.ignoreButton setTitle:[NSLocalizedString(@"ignore_underline", nil) stringWithSentenceCapitalization] forState:UIControlStateNormal];
                [cell.acceptButton setTitle:[NSLocalizedString(@"accept_underline", nil) stringWithSentenceCapitalization] forState:UIControlStateNormal];
            }
        ...
        if ([afriend hasFriendImageAssigned]) {
            ...
            [cell setImageForFriend:afriend withEncryptionParams: ep placeholderImage:  [UIImage imageNamed:@"app_icon"] progress:^(NSUInteger receivedSize, long long expectedSize) {
            ...
            cell.friendImage.image = [UIImage imageNamed:@"user_placeholder"];
        ...
        if (messages.count == 0) {
            ...
            static NSString *CellIdentifier = @"Cell";
            UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
            cell.textLabel.text = [NSLocalizedString(@"no_messages", nil) stringWithSentenceCapitalization];
            cell.textLabel.textColor = [self getTextColor];
            cell.backgroundColor = [UIColor clearColor];
            cell.textLabel.textAlignment = NSTextAlignmentCenter;
            cell.userInteractionEnabled = NO;
        ...
        if (messages.count > 0 && indexPath.row < messages.count) {
            ...
            cell.bubbleView.backgroundColor = ours ? [UIColor colorWithString:@"c7f3ca"] : [UIColor colorWithString:@"c7edfc"];
            ...
            if (message.errorStatus > 0) {
                ...
                if (message.serverid <= 0) {
                    ...
                    cell.messageStatusLabel.text = [NSLocalizedString(@"message_sending",nil) stringWithSentenceCapitalization];
                }
                else {
                    ...
                    if ((!message.plainData && [message.mimeType isEqualToString:MIME_TYPE_TEXT]) ||
                        ...
                        cell.messageStatusLabel.text = [NSLocalizedString(@"message_loading_and_decrypting",nil) stringWithSentenceCapitalization];
                    }
                    else {
            ...
            if ([message.mimeType isEqualToString:MIME_TYPE_TEXT]) {
                ...
                if ([message.mimeType isEqualToString:MIME_TYPE_IMAGE]) {
                    ...
                    CGRect messageStatusFrame = cell.messageStatusLabel.frame;
                    messageStatusFrame.origin.x = 24;
...
-(void) loadChat:(NSString *) username show: (BOOL) show  availableId: (NSInteger) availableId availableControlId: (NSInteger) availableControlId {
    ...
    if (!chatView) {
        ...
        [chatView setSeparatorColor: [UIColor clearColor]];
        [chatView setTableFooterView:[[UIView alloc] init]];
            ...
            [[ChatController sharedInstance] loadEarlierMessagesForUsername: username callback:^(id result) {
                if (result) {
                    ...
                    if (resultValue == 0 || resultValue == NSIntegerMax) {
                        ...
                        [self updateTableView:weakView withNewRowCount:(int)[result intValue]];
                    }
                }
                else {
...
-(void) updateKeyboardState: (BOOL) goingHome {
    DDLogInfo(@"updateKeyboardState, goingHome: %d", goingHome);
    if (goingHome) {
        [self resignAllResponders];
    }
    else {
...
- (void)refreshMessages:(NSNotification *)notification {
    ...
    DDLogVerbose(@"username: %@, currentchat: %@, scroll: %d", username, [self getCurrentTabName], scroll);
...
-(REMenu *) createMenuMenu {
    ...
    if ([self getCurrentTabName]) {
        ...
        if ([theFriend isFriend] && ![theFriend isDeleted]) {
            ...
            REMenuItem * selectImageItem = [[REMenuItem alloc]
                                            initWithTitle:[NSLocalizedString(@"select_image", nil) stringWithSentenceCapitalization]
            ...
            REMenuItem * captureImageItem = [[REMenuItem alloc]
                                             initWithTitle:[NSLocalizedString(@"capture_image", nil) stringWithSentenceCapitalization]
        ...
        REMenuItem * closeTabItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_close_tab", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_end_conversation"] highlightedImage:nil action:^(REMenuItem * item){
            [self closeTab];
        }];
        ...
        REMenuItem * deleteAllItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_delete_all_messages", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_delete"] highlightedImage:nil action:^(REMenuItem * item){
            ...
            if (confirm) {
                NSString * okString = [NSLocalizedString(@"ok", nil) stringWithSentenceCapitalization];
                [UIAlertView showWithTitle:[NSLocalizedString(@"delete_all_title", nil) stringWithSentenceCapitalization]
                                   message:[NSLocalizedString(@"delete_all_confirmation", nil) stringWithSentenceCapitalization]
                         cancelButtonTitle:[NSLocalizedString(@"cancel", nil) stringWithSentenceCapitalization]
    ...
    REMenuItem * shareItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"share_invite_link", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"blue_heart"] highlightedImage:nil action:^(REMenuItem * menuitem){
        
        _progressView = [LoadingView showViewKey:@"invite_progress_text"];
        NSString * inviteUrl = @"http://wildbeastbitcoin.com/apps";
        
        [[NetworkController sharedInstance] getShortUrl:inviteUrl callback:^(id shortUrl) {
            [_progressView removeView];
            NSString * text = [NSString stringWithFormat:[NSLocalizedString(@"external_invite_message", nil) stringWithSentenceCapitalization], shortUrl];
    ...
    /*REMenuItem * pwylItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"pay_what_you_like", nil) stringWithSentenceCapitalization] image:
    ...
    if (![[PurchaseDelegate sharedInstance] hasVoiceMessaging]) {
        REMenuItem * purchaseVoiceItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_purchase_voice_messaging", nil) stringWithSentenceCapitalization] image:
    ...
    REMenuItem * settingsItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"settings", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_preferences"] highlightedImage:nil action:^(REMenuItem * item){
        [self showSettings];
    ...
    REMenuItem * logoutItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"logout", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_lock_power_off"] highlightedImage:nil action:^(REMenuItem * item){
        [self logout];
...
-(REMenu *) createHomeMenuFriend: (Friend *) thefriend {
    ...
    if ([thefriend isFriend]) {
        ...
        if ([thefriend isChatActive]) {
            REMenuItem * closeTabHomeItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_close_tab", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_end_conversation"] highlightedImage:nil action:^(REMenuItem * item){
        ...
        REMenuItem * deleteAllHomeItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_delete_all_messages", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_delete"] highlightedImage:nil action:^(REMenuItem * item){
        ...
        if (![thefriend isDeleted]) {
            REMenuItem * fingerprintsItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"verify_key_fingerprints", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"fingerprint_zoom"] highlightedImage:nil action:^(REMenuItem * item){
                //cameraUI
            ...
            if (![thefriend hasFriendImageAssigned]) {
                REMenuItem * selectImageItem = [[REMenuItem alloc]
                                                initWithTitle:[NSLocalizedString(@"menu_assign_image", nil) stringWithSentenceCapitalization]
                                                image:[UIImage imageNamed:@"ic_menu_gallery"]
                                                highlightedImage:nil
                                                action:^(REMenuItem * item){
                ...
                REMenuItem * removeImageItem = [[REMenuItem alloc]
                                                initWithTitle:[NSLocalizedString(@"menu_remove_friend_image", nil) stringWithSentenceCapitalization]
                                                image:[UIImage imageNamed:@"ic_menu_gallery"]
                                                highlightedImage:nil
                                                action:^(REMenuItem * item){
            ...
            if (![thefriend hasFriendAliasAssigned]) {
                REMenuItem * assignAliasItem = [[REMenuItem alloc]
                                                initWithTitle:[NSLocalizedString(@"menu_assign_alias", nil) stringWithSentenceCapitalization]
                                                ...
                                                action:^(REMenuItem * item){
                                                    ...
                                                    UIAlertView * av = [[UIAlertView alloc]
                                                                        initWithTitle:[NSString stringWithFormat:[NSLocalizedString(@"enter_alias", nil) stringWithSentenceCapitalization], [thefriend name]]
                                                                        message:[NSString stringWithFormat:[NSLocalizedString(@"enter_alias_for", nil) stringWithSentenceCapitalization], [thefriend name]]
                                                                        delegate:self cancelButtonTitle:[NSLocalizedString(@"cancel", nil) stringWithSentenceCapitalization]
                                                                        otherButtonTitles:[NSLocalizedString(@"ok", nil) stringWithSentenceCapitalization], nil];
                ...
                REMenuItem * removeAliasItem = [[REMenuItem alloc]
                                                initWithTitle:[NSLocalizedString(@"menu_remove_friend_alias", nil) stringWithSentenceCapitalization]
                                                image:[UIImage imageNamed:@"ic_menu_friendslist"]
                                                highlightedImage:nil
                                                action:^(REMenuItem * item){
    ...
    if (![thefriend isInviter]) {
        ...
        REMenuItem * deleteFriendItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_delete_friend", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_blocked_user"] highlightedImage:nil action:^(REMenuItem * item){
            
            NSString * okString = [NSLocalizedString(@"ok", nil) stringWithSentenceCapitalization];
            [UIAlertView showWithTitle:[NSLocalizedString(@"menu_delete_friend", nil) stringWithSentenceCapitalization]
                               message:[NSString stringWithFormat: [NSLocalizedString(@"delete_friend_confirmation", nil) stringWithSentenceCapitalization], [UIUtils buildAliasStringForUsername:thefriend.name alias:thefriend.aliasPlain]]
                     cancelButtonTitle:[NSLocalizedString(@"cancel", nil) stringWithSentenceCapitalization]
                     otherButtonTitles:@[okString]
                              tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
                                  if (buttonIndex == [alertView cancelButtonIndex]) {
...
-(REMenu *) createChatMenuMessage: (SurespotMessage *) message {
    ...
    if ([message.mimeType isEqualToString:MIME_TYPE_TEXT] && message.plainData) {
        REMenuItem * copyItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_copy", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_copy"] highlightedImage:nil action:^(REMenuItem * item){
    ...
    if ([message.mimeType isEqualToString:MIME_TYPE_IMAGE]) {
        if (message.errorStatus > 0 && ours) {
            ...
            title = [NSLocalizedString(@"menu_resend_message", nil) stringWithSentenceCapitalization];
        ...
        if (message.serverid > 0 && ours) {
            ...
            NSString * title = nil;
            if (!message.shareable) {
                title = [NSLocalizedString(@"menu_unlock", nil) stringWithSentenceCapitalization];
                image = [UIImage imageNamed:@"ic_menu_partial_secure"];
            }
            else {
                title = [NSLocalizedString(@"menu_lock", nil) stringWithSentenceCapitalization];
        ...
        if (message.shareable && !ours) {
            REMenuItem * saveItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"save_to_photos", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_save"] highlightedImage:nil action:^(REMenuItem * item){
                if (message.shareable && !ours) {
    ...
    //can always delete
    REMenuItem * deleteItem = [[REMenuItem alloc] initWithTitle:[NSLocalizedString(@"menu_delete_message", nil) stringWithSentenceCapitalization] image:[UIImage imageNamed:@"ic_menu_delete"] highlightedImage:nil action:^(REMenuItem * item){
        ...
        if (confirm) {
            NSString * okString = [NSLocalizedString(@"ok", nil) stringWithSentenceCapitalization];
            [UIAlertView showWithTitle:[NSLocalizedString(@"delete_message", nil) stringWithSentenceCapitalization]
                               message:[NSLocalizedString(@"delete_message_confirmation_title", nil) stringWithSentenceCapitalization]
                     cancelButtonTitle:[NSLocalizedString(@"cancel", nil) stringWithSentenceCapitalization]
                     otherButtonTitles:@[okString]
                              tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
                                  if (buttonIndex == [alertView cancelButtonIndex]) {
...
-(void) invite: (NSNotification *) notification {
    ...
    if (currentChat) {
        [UIUtils showToastMessage:[NSString stringWithFormat:[NSLocalizedString(@"notification_invite", nil) stringWithSentenceCapitalization], [[IdentityController sharedInstance] getLoggedInUser], thefriend.nameOrAlias] duration:1];
...
- (void)settingsViewController:(IASKAppSettingsViewController*)sender buttonTappedForSpecifier:(IASKSpecifier*)specifier {
    ...
    if ([specifier.key isEqualToString:@"_user_assign_background_image_key"]) {
        ...
        if (bgImageUrl) {
            NSString * assignString = [NSLocalizedString(@"pref_title_background_image_select", nil) stringWithSentenceCapitalization];
...
-(void) setTextBoxHints {
    ...
    if (tbHintCount++ < 6) {
        [_inviteTextView setPlaceholder:[NSLocalizedString(@"invite_hint", nil) stringWithSentenceCapitalization]];
        [_messageTextView setPlaceholder:[NSLocalizedString(@"message_hint", nil) stringWithSentenceCapitalization]];
    }
