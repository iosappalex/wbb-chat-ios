...
-(ChatDataSource*)initWithUsername:(NSString *) username loggedInUser: (NSString * ) loggedInUser availableId:(NSInteger)availableId availableControlId:( NSInteger) availableControlId {
    ...
    DDLogVerbose(@"username: %@, loggedInUser: %@, availableid: %ld, availableControlId: %ld", username, loggedInUser, availableId, (long)availableControlId);
    //call super init
    self = [super init];
            ...
            for (SurespotMessage * message in messages) {
                ...
                [self addMessage:message refresh:NO callback:^(id result) {
                    if ([weakSelf.decryptionQueue operationCount] == 0) {
                        DDLogInfo(@"loaded %lu messages from disk at: %@", [messages count] ,path);
...
-(void) postRefreshScroll: (BOOL) scroll {
    DDLogInfo(@"postRefreshScroll username: %@, %d", _username, scroll);
    [self sort];
