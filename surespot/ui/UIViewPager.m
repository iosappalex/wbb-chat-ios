...
- (id) initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self setBackgroundColor:[UIColor colorWithString:NAVBAR_COLOR]];
        UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapped:)];
        [self addGestureRecognizer:tapGestureRecognizer];
        
        
        _homeView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ic_menu_home_blue"]];
        _homeView.contentMode = UIViewContentModeScaleAspectFit;
        _homeView.frame = CGRectMake(0, 0, CGRectGetHeight(self.frame), CGRectGetHeight(self.frame));
